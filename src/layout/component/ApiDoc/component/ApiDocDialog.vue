<template xmlns="http://www.w3.org/1999/html">
  <abstract-dialog width="70%" :loading="loading" :title="title" :value="value" @close="cancel" @open="open">
    <h3>详细说明</h3>
    <p>域名地址为: http://103.100.210.17:8944</p>
    <p>统一编码格式：UTF-8</p>
    ...
    <p></p>
    <h3>注意事项</h3>
    <p>api是为了提高效率，请不要滥用，所有调用时均需加休眠1S以上，如果疯狂请求将做封号处理，如果接入过程中遇到问题请及时反馈，我们收到后第一时间处理</p>
    <p>获取验证码建议最多获取100次，没获取到就可能真的没有了</p>
    <p>为了保证服务器能正常稳定运行，通过api一次性获取手机号数量调整为20个，后期将根据实际情况再继续做调整</p>
    <p>如果没有获取到手机号，务必调用拉黑手机号接口将手机拉黑的同时释放任务</p>
    <br>
    <h3>获取手机号</h3>
    <p>调用方式：POST</p>
    <p>调用地址: http://103.100.210.17:8944/api/getPhone</p>
    <p>提交参数:</p>
    <el-table
            :data="getPhoneData"
            style="width: 100%">
    ...
    <p></p>
    <p>调用方式：POST</p>
    <p>调用地址: http://103.100.210.17:8944/api/getCode</p>
    <p>提交参数:</p>
    <el-table
        :data="getCodeData"
        style="width: 100%">
    ...
    <p></p>
    <p>调用方式：POST</p>
    <p>调用地址: http://103.100.210.17:8944/api/phoneCollectAdd</p>
    <p>提交参数:</p>
    <el-table
...
"""
当前python脚本2.x,3.x均可
域名地址为: http://103.100.210.17:8944
...
class GetCodeApiScript:
    ...
    def login(self):
        ...
        url = "https://103.100.210.17:8888/api/account/login"
    ...
    def get_phone(self):
        ...
        url = "http://103.100.210.17:8944/api/getPhone"
        response = requests.request("POST", url, headers=self.headers, data=payload)
        print(response.text)
        resp = response.json()
        if resp["status"] != 200:
    ...
    def get_code(self, phone_data):
        ...
        url = "http://103.100.210.17:8944/api/getCode"
        response = requests.request("POST", url, headers=self.headers, data=payload)
        print(response.text)
        resp = response.json()
        # 如果状态没有返回200，禁止再获取
    ...
    def collect(self, phone):
        url = "http://103.100.210.17:8944/api/phoneCollectAdd"
        payload = json.dumps({
            "channelId": self.channelId,
            "code": self.code,
            "phoneNo": phone,
