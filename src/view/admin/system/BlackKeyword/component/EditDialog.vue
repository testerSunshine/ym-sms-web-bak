  ...
  <abstract-dialog :loading="loading" :title="title" :value="value" @close="cancel" @open="open">
    <abstract-form :model="form" :rules="rules" label-width="90px">
      <el-form-item label="敏感词" prop="keyword">
        <el-input v-model="form.keyword" :readonly="!!form.id || !canEdit" maxlength="20"/>
      </el-form-item>
...
import {add, update} from "@/api/system/BlackKeyword"
...
export default {
  ...
  computed: {
    ...
    confirmMessage() {
      switch (this.type) {
        case 'add':
          return `确认添加新的敏感词【${this.form.account}】?`
        case 'edit':
          return `确认提交对【${this.data.keyword}】作出的修改？`
        default:
          return ''
      }
    },
